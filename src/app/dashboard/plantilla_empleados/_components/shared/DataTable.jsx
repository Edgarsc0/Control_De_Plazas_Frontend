 ...
 * @param {Array<{key: string, label: string, width: number, visible: boolean, noFilter?: boolean, greenHeader?: boolean}>} props.columns - Columnas (todas; se filtran las visibles internamente). `noFilter` marca columnas sin dato filtrable (ej. la de fotografía): se les oculta el orden, el botón de filtro y el buscador de texto del encabezado. `greenHeader` pinta el encabezado de esa columna en verde (grupos visuales definidos por el tab, ej. columnas AL-AV de Plantilla Detalle) — cede ante el resaltado de selección/filtro activo.
            ...
            <>
              <tr>
                ...
                {headerGroupRuns.map((run) => {
                  ...
                  const bgClass = isColSelected(index) ? "bg-[#621f32] text-white" : (hasFilter ? "bg-[#bc955c] text-slate-900 shadow-inner" : (col.greenHeader ? "bg-emerald-800 dark:bg-emerald-900 text-emerald-50" : "bg-[#501929] text-slate-200"));
